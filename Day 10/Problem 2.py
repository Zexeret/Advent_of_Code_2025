...
# Just an code sample taken from outside reference
# keeping it here in case you need to solve it. 
# using c++ required external libraries, which I didnt want right now.
def get_data(fname):
    a = []
    p = re.compile('\[([.#]+)\](.*)\{(.*)\}.*')
